...
class question_list implements \Countable, \ArrayAccess, \Iterator
{
     ...
     * @return boolean
     */
    public function validate(Quiz $quiz) {
        $this->errors = array();
        if (count($this->getIds()) != $quiz->qnumber) {
            $this->errors['qnumber'] = 'validateql_wrong_number';
        }
        $scores = $this->getScores();
        if (abs(array_sum($scores) - $quiz->score) > 0.01) {
    ...
    public static function fromJson($json) {
        ...
        if (!empty($decoded) && is_array($decoded)) {
            foreach ($decoded as $q) {
                ...
                // Section header.
                if (is_string($q)) {
                    $new = new \mod_automultiplechoice\local\models\question_section($q);
                } else if (is_array($q)) { // Real Question.
                    if (isset($q[0])) {
                        $new = \mod_automultiplechoice\local\models\question::fromArray(
                            array(
                                'id' => (int) $q[0],
                                'score' => (double) $q[1],
                    ...
                    if ($q->model === 'QuestionSection') {
                        $new = new \mod_automultiplechoice\local\models\question_section($q->name, $q->description);
                    } else if ($q->model === 'Question') {
                        $new = \mod_automultiplechoice\local\models\question::fromArray((array) $q);
                    }
                }
                if ($new) {
                    $qlist->questions[] = $new;
