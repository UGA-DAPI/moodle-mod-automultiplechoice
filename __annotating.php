...
$process = new \mod_automultiplechoice\local\amc\annotate($quizz);
$associateprocess =  new \mod_automultiplechoice\local\amc\associate($quizz);
if ($action === 'annotate') {
...
$warnings =   \mod_automultiplechoice\local\helpers\log::build($quizz->id)->check('annotating');
if ($warnings) {
...
if ($process->countAnnotatedFiles()>0) {
   $url = $associateprocess->getFileActionUrl( $process->normalizeFilename('corrections'));
    ...
    if (empty($idnumber) and empty($copy)) {
       ...
       foreach ($usersdisplay as $user){
        ...
        $name = $user;
           ...
           $copy = explode('_',$name);
           $thumbnailimg = \html_writer::img($process->getFileRealUrl('name-'.$name.".jpg"),$name);
...
//var_dump($pages);     error($pages);
    foreach ($pages as $page){
        echo \html_writer::img($process->getFileRealUrl(basename($page)),$page);
    }
